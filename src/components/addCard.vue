<template>
	<div class="mt-addCard">
		<group title="请添加观影券/现金券/优惠码">
			<x-input title="账 号" type="number" disabled v-model="cardNum"></x-input>
			<x-input title="序列号" type="number" disabled v-model="serialNumber"></x-input>
...
<script>
	...
	export default{
		...
		data(){
			return{
				cardNum:this.$route.params.card_no,
				serialNumber:this.$route.params.card_number,
				pw:"",
			}
		},
		methods:{
